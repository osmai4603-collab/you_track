...
class TagRemoteDatasourceImpl implements TagRemoteDatasource {
  ...
  Future<TagModel> createTag({
    ...
    for (final entry in permissions.entries) {
      final permissionResponse = await supabase.from('tag_permissions').insert({
        'tag_id': tagId,
      ...
      if (entry.value == 'specific_users' && specificUserIds != null) {
        final userPermissionData = specificUserIds.map((userId) => {
    ...
    if (subscriptionEvents.isNotEmpty) {
      final subscriptionData = subscriptionEvents.map((event) => {
        'tag_id': tagId,
        'event_type': event,
      }).toList();
    ...
    final fullTagResponse = await supabase
        .from('tags')
        .select('*, tag_permissions(*, tag_permission_users(*)), tag_subscriptions(*)')
        .eq('id', tagId)
        .single();
