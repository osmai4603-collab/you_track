...
// ── AI Features Tab ─────────────────────────────────────────
class _AIFeaturesTab extends StatelessWidget {
  final ColorScheme colors;
  final TextTheme textTheme;

  const _AIFeaturesTab({required this.colors, required this.textTheme});
...
class _GroupsTab extends StatefulWidget {
  ...
  const _GroupsTab({required this.colors, required this.textTheme});
      ...
      builder: (context, state) {
        ...
        return Column(
          children: [
            ...
            const Divider(height: 1),
            _GroupsTableHeader(colors: colors, textTheme: widget.textTheme),
            const Divider(height: 1),
            ...
            if (groups.isEmpty && state is GroupsLoaded)
              const Padding(
                padding: EdgeInsets.all(24),
...
class _GroupsTableHeader extends StatefulWidget {
  final ColorScheme colors;
  final TextTheme textTheme;

  const _GroupsTableHeader({required this.colors, required this.textTheme});

  @override
  State<_GroupsTableHeader> createState() => _GroupsTableHeaderState();
}

class _GroupsTableHeaderState extends State<_GroupsTableHeader> {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 100, child: Text('Members')),
          SizedBox(width: 100, child: Text('Roles')),
          SizedBox(width: 120, child: Text('Projects')),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 100, child: Text(widget.members)),
      ...
      builder: (context, state) {
        ...
        return Column(
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const Text('View by:'),
                  const SizedBox(width: 8),
                  _ToggleButton(label: 'Roles', isSelected: true),
                  const SizedBox(width: 4),
                  _ToggleButton(
            ...
            const Divider(height: 1),
            _RolesTableHeader(colors: colors, textTheme: textTheme),
            const Divider(height: 1),
            if (state is RolesLoading)
              ...
              ...roles.map(
                (r) => Column(
                  children: [
                    _RoleRow(role: r.name, project: r.permissions.join(', ')),
                    const Divider(height: 1),
                  ],
                ),
              ),
            if (roles.isEmpty && state is RolesLoaded)
              const Padding(
                padding: EdgeInsets.all(24),
...
class _RolesTableHeader extends StatefulWidget {
  final ColorScheme colors;
  final TextTheme textTheme;

  const _RolesTableHeader({required this.colors, required this.textTheme});

  @override
  State<_RolesTableHeader> createState() => _RolesTableHeaderState();
}

class _RolesTableHeaderState extends State<_RolesTableHeader> {
  bool _allSelected = false;
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 300, child: Text('Project')),
          const SizedBox(width: 40),
        ],
      ),
    );
...
class _RoleRow extends StatefulWidget {
  ...
  const _RoleRow({required this.role, required this.project});
